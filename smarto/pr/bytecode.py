...
def disassemble(bytecode: str) -> Tuple[List[str], Dict[str, str], Dict[str, Any]]:
    """Disassemble EVM bytecode with function signature detection and security analysis."""
    if not bytecode:
        return [], {}, {'dangerous_opcodes': {}}
    ...
    # First pass - disassemble and collect potential selectors
    while i < len(bytecode):
        ...
        if mnemonic.startswith("PUSH"):
            n = int(mnemonic[4:]) if mnemonic != "PUSH0" else 0
            value = bytecode[i : i + n * 2].ljust(n * 2, "0") if n > 0 else ""

            if n > 0:
                # Function signature detection for PUSH4
                if n == 4 and len(value) == 8:
                    potential_selectors.append(value)

                args = f"0x{value}" if value else ""
                i += len(value)
                pc += n

            output.append(f"{pc_hex} {mnemonic} {args}".strip() + comment)
        else:
            output.append(f"{pc_hex} {mnemonic}")

        pc += 1

    # Get function signatures for all found selectors
    signatures = {}

    print(f"Found {len(potential_selectors)} potential selectors")

    # Fetch unknown signatures in parallel
    if potential_selectors:
        fetched = fetch_signatures_parallel(potential_selectors)
        for selector, signature in fetched.items():
            if signature:
                signatures[selector] = signature

    # Second pass - add signature comments
    for i, line in enumerate(output):
        for selector, signature in signatures.items():
            if f"PUSH4 0x{selector}" in line:
                output[i] = f"{line} # {signature}"
                break
    
    # Detect potentially dangerous opcodes
    security_info = {
        'dangerous_opcodes': detect_dangerous_opcodes(output)
    }

    return output, signatures, security_info
